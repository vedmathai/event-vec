...
class QuestionAnsweringBase(nn.Module):
    ...
    def __init__(self):
        ...
        self._model = BigBirdModel.from_pretrained('google/bigbird-roberta-base', attention_type="original_full")
        modules = [self._model.embeddings, *self._model.encoder.layer[:-3]]
        for module in modules:
            for param in module.parameters():
                param.requires_grad = False
        self._dropout = nn.Dropout(0.5)
        self._token_classifier_1 = torch.nn.Linear(768, 16)
        self._token_classifier_activation = nn.Tanh()
        self._token_classifier_2 = torch.nn.Linear(16, 2)
        self._tense_classifier = torch.nn.Linear(16, 4)
